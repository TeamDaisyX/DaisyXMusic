...
async def playlist(client, message):
    ...
    msg = "**TUNGGU YA SAYANG...** in {}".format(message.chat.title)
    msg += "\n- "+ now_playing
    msg += "\n- Req by "+by
    temp.pop(0)
    if temp:
...
async def p_cb(b, cb):
    ...
    if type_ == 'playlist':           
        ...
        now_playing = temp[0][0]
        by = temp[0][1].mention(style='md')
        msg = "**MUSIC** in {}".format(cb.message.chat.title)
        msg += "\n- "+ now_playing
        msg += "\n- Req by "+by
        temp.pop(0)
        if temp:
...
async def m_cb(b, cb):
    ...
    if type_ == 'pause':
        if (
            ...
            await cb.answer('MUSIC DIHENTIKAN!')
...
async def play(_, message: Message):
    ...
    for administrator in administrators:
       if administrator == message.from_user.id:  
               ...
               try:
                   ...
                   #print(e)
                   #await lel.edit(
                   #    f"<b>User {user.first_name} MASUKKAN @musicsrobot_bot dan @robotassisten DULU SAYANG! JANGAN DIKICK DARI GRUB NANTI MASUK DAFTAR BLOKIR."
    ...
    await lel.edit("🔎 **TUNGGU YA SAYANG... **") 
    ...
    try:
        ...
        await lel.edit("⚠️CARI YANG BENER!")
        print(str(e))
        return
    ...
    if message.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        photo="final.png", 
        caption=f"🎬 MUSIC MASUK **ANTRIAN** KE {position}!",
        ...
        photo="final.png",
        reply_markup=keyboard,
        caption="▶️ **MEMUTAR** MUSIC REQUEST DARI {} via **ROBOT MUSIC** 🎉".format(
...
async def deezer(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **TUNGGU YA SAYANG...**")
...
async def jiosaavn(client: Client, message_: Message):
    ...
    if message_.chat.id in callsmusic.pytgcalls.active_calls:
        ...
        await res.edit_text("▶️ TUNGGU YA SAYANG...")
        chat_id = message_.chat.id
        que[chat_id] = []
        qeue = que.get(message_.chat.id)
        s_name = sname
